...
`first_snapshot_eligible` (caller-supplied) exists because "the very first
save ever" isn't actually the right definition of "first look" — two
different reasons, from two different callers:

- **The "Regenerate picks" button** (a human click) uses
  `picks_core.is_first_look_window()`, `True` within `FIRST_LOOK_WINDOW_DAYS`
  (3) days *before* that week's earliest kickoff through
  `FIRST_LOOK_LATE_GRACE_DAYS` (1) day *after* it. The season/week selector
  lets you preview any week at any time, and a save made while browsing
  ahead (checking what week 10 looks like while week 3 is current)
  shouldn't get permanently recorded as week 10's first real review — this
  window matches the actual usage pattern instead: check a few days before
  ...
  outside the window still saves normally as `'current'`; it just can't
  claim `'first'`.
- **`resolve_week_lock()`'s fallback save** (an unattended, deadline-passed
  auto-lock — this is a single-user app with no background scheduler, so
  the fallback only ever runs on the next page load, whenever that is)
  always passes `first_snapshot_eligible=False`, unconditionally, on both
  of its branches — no date window involved at all. A save made here locks
  the week immediately, so it can never be followed by a second, differing
  save to compare a `'first'` snapshot against: on the reused-snapshot
  path, `'first'` was either already captured back when that snapshot was
  originally generated by the button above, or never will be, since
  nothing else was ever saved for the week; on the fresh-computation path,
  this is definitionally the week's only save, so a `'first'` row would be
  permanently identical to `'current'` regardless of how close to kickoff
  it happened to land — capturing one would waste a row and imply a real
  comparison exists when it never will.

**A `(season_year, week)` that only ever gets locked by the fallback save
(nobody ever clicked "Regenerate picks" before the deadline) ends up with
only a `'current'` row, permanently.** The Picks tab's snapshot toggle
simply doesn't render for that week (nothing to compare against), rather
than showing a `'first'` that's a pointless, permanent duplicate of
`'current'`.
