...
- **Team timeline / power-timeline read** (`team_power_timeline_scores()`)
  ...
  *continuous* score rather than a fixed two- or three-point label — display
  ...
  - **Actual record** — real `wins / (wins + losses + ties)` from Sleeper's
    ...
    Shrunk toward that same `0.5` early in the season
    (`_shrunk_win_pct()`, weight `games_played / (games_played +
    WIN_PCT_SHRINKAGE_K)`), so a 1-0/0-1 start doesn't swing the z-score as
    hard as a settled record does — an undiscounted raw win_pct would treat
    a coin-flip result as fully resolved off a single game.
...
| `WIN_PCT_SHRINKAGE_K = 4` (games worth of shrinkage weight toward `0.5`) | `dynasty_core.py` | A judgment call for how fast the power/timeline read's `win_pct` should trust a real record over the neutral prior — not derived from any league setting or statistical fit | Adjust by feel if early-season `power_score` still looks too swingy or too damped |
