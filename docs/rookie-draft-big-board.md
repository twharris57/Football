...
- The "This week's projection" mode's scoring dot product only covers
  ...
  league's `scoring_settings` — it currently misses `bonus_rec_te` (TE
  premium), which is a position-conditional weight, not a raw stat category
  Sleeper's global (non-league-scoped) projections endpoint could ever
  emit directly (see `.claude/PROJECT_PLAN.md`'s fix-before-merge section
  and `valuation_principles.md`'s "generic stat-vocabulary dot product"
  rule). Every TE's weekly-projection score is undercounted until fixed.
...
| `GET /projections/nfl/regular/{season}/{week}`'s path shape and its stat-key vocabulary lining up 1:1 with `league["scoring_settings"]` for ordinary counting stats | `sleeper_api.get_weekly_projections`, `dynasty_core/lineup.py`'s `_weekly_projected_points` | An undocumented, unofficial Sleeper endpoint — no contract to rely on. Confirmed live (2026-08-21) to return real per-stat projections in the same stat-key vocabulary this league's `scoring_settings` already uses for raw counting stats — but this does not extend to position-conditional weights like `bonus_rec_te`, which the endpoint (global, not league-scoped) can never emit as its own key (see `valuation_principles.md`'s "generic stat-vocabulary dot product" rule). Wrapped in its own try/except (`gather_state`) so a fetch failure degrades to a `data_warnings` entry and an "unavailable" Lineup-tab mode, not a crash — but a malformed value *within* a successful fetch is not yet defended against (`.claude/PROJECT_PLAN.md`'s `VA-7`) | Re-verify the live response shape if the weekly-projection lineup mode ever starts looking systematically wrong; extend the position-conditional handling if `scoring_settings` ever gains another non-raw-stat weight beyond `bonus_rec_te` |
