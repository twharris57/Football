"""Persist the free-agent pool's team/depth-chart/status across refreshes to detect
in-season pickup opportunities week-over-week.
...
Sleeper's own player data (no new fetch): `team` (a free agent signing with a real NFL
team), `depth_chart_order` (a lower number is better - closer to starting), and
`status` (transitioning to "Active"). Only improvements are flagged for the latter two
- a demotion isn't a pickup opportunity.
...
def _diff(previous_players: dict[str, dict], pool: dict[str, dict]) -> list[dict[str, Any]]:
    """Pure: compare each pool player's current team/depth_chart_order/status
    against their last-known values. No disk I/O.

    A player with no entry in `previous_players` is treated as a real "just
    entered the pool" event (kind "team", old=None) - not skipped. Skipping
    ...
    The true first-ever-run case (nothing has ever been tracked at all) is
    handled one level up, in reconcile_pickup_snapshot, before this is called.
    """
    changes = []
    for player_id, info in pool.items():
...
def reconcile_pickup_snapshot(
    league_id: str, season: str, pool: dict[str, dict]
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Load, diff, persist-if-changed, return (updated_snapshot, changes).

    `pool` is typically free_agent_pool()'s output. Keyed by `league_id` and
    `season` together - draft_snapshots.py gets season-rollover safety for
    free from draft_id changing every year automatically; whether Sleeper's
    league_id itself rotates per season for a continuing dynasty league
    wasn't verified against this project's own data, so the season suffix is
    a cheap hedge either way, not a real cost.

    Snapshot entries are merged, not replaced: a player who leaves the pool
    (drafted by a fantasy team, loses their NFL roster spot) keeps their
    last-known values on file, so a real change is still detectable if/when
    they re-enter the pool later rather than silently resetting to "first
    sighting" again.
    """
    ...
    current_players = {
        player_id: {field: info.get(field) for field in _TRACKED_FIELDS} for player_id, info in pool.items()
    }

    if existing["initialized"]:
        changes = _diff(existing["players"], pool)
    else:
