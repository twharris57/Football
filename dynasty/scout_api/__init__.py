...
feature (see .claude/PROJECT_PLAN_DYNASTY.md's SC-15).
...
Originally an inbound FastAPI server (SC-11's PoC slice, `/health`/`/ping`)
the cloud routine would call directly - retired after two nights of live
debugging proved this network's inbound path from the cloud sandbox to
this NAS cannot work (see PROJECT_PLAN_DYNASTY.md's "Why the inbound
design was abandoned"). A separate deployable image from
